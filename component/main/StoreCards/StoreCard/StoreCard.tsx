...
const StoreCard = ({id,image,title,rate,subtitle}:typeItem) => {
    return (
        <Grid key={id} container item xs={12} md={4} display={'flex'} height={{xs:'90px', md:'120px'}} sx={{ border: '1px solid #e0e0e0', borderRadius:{xs:'3px', md:'8px'}, padding:'10px','&:hover':{boxShadow:'1'}}} alignItems={'center'}>
            ...
            <Grid container item xs={8} md={9} display={'flex'} justifyContent={'center'} sx={{ padding:{xs:'4px', md:'10px'}, gap:{xs:'2px', ms:'8px'} }}>
                ...
                <Grid container item xs={12} display={'flex'} gap={'5px'}>
                    <Grid xs={10} container item display={'flex'} justifyContent={'flex-start'} alignItems={'center'}>
                        <Grid item xs={1} width={{xs:'20px', md:'30px'}} height={{xs:'15px', md:'25px'}}>
                    ...
                    <Grid item xs={1} sx={{ display: 'flex', justifyContent: 'flex-end',cursor:'pointer',color:'#757575' }}>
